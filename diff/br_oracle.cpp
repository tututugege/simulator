...
RefCpuState dut_to_ref_state(uint8_t privilege) {
  ...
  state.inst_idx = dut_cpu.inst_idx;
  state.commit_pc = dut_cpu.commit_pc;
...
void ensure_oracle() {
  if (oracle_ctx == nullptr) {
    oracle_ctx = refcpu_init(0, static_cast<uint32_t>(RAM_SIZE));
...
inline void seed_oracle_io_from_backing() {
  ...
  for (const auto &range : kCkptIoRanges) {
    for (uint32_t off = 0; off + 4u <= range.size; off += 4u) {
      ...
      refcpu_set_io_word(oracle_ctx, addr, data);
    }
  }
}
...
void init_oracle(int img_size) {
  ...
  refcpu_sync_ram_from_dut(oracle_ctx, pmem_ram_ptr(),
...
void init_oracle_ckpt(CPU_state ckpt_state, uint8_t privilege) {
  ...
  state.inst_idx = ckpt_state.inst_idx;
  state.commit_pc = ckpt_state.commit_pc;
  ...
  refcpu_sync_ram_from_dut(oracle_ctx, ckpt_memory,
                           static_cast<size_t>(RAM_SIZE));
  seed_oracle_io_from_backing();
  (void)refcpu_probe_pc_translation(oracle_ctx);
}
...
void get_oracle(struct front_top_in &in, struct front_top_out &out) {
  ...
  out.FIFO_valid = true;
  out.predict_next_fetch_address = current_oracle_state().pc;
  for (i = 0; i < FETCH_WIDTH; i++) {
    RefCpuState state = current_oracle_state();
    ...
    if (!kOracleSteadyFetchWidth &&
        ((state.pc ^ out.pc[i]) & ~(ICACHE_LINE_SIZE - 1))) {
      break;
    }
  }
