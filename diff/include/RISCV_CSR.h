...
void RISCV_CSR(bool input_data[bit_width], bool* output_data) {
	...
	//S_interrupt条件：1、原本到M态的中断被给到了S态处理（此时global interrupt被视为S态的，即特权级小于S或等于S且SIE为1）。2、本身是到S态的中断，需要privilege小于S或等于S且SIE为1。
	bool S_software_interrupt = (csr[number_mip][31 - 3] && csr[number_mie][31 - 3] && csr[number_mideleg][31 - 3] == 1 && number_this_priviledge < 2 && (number_this_priviledge < 1 || csr[number_mstatus][31 - 1] == 1)) ||
		(csr[number_mip][31 - 1] && csr[number_mie][31 - 1] && number_this_priviledge < 2 && (number_this_priviledge < 1 || csr[number_mstatus][31 - 1] == 1));
	bool S_timer_interrupt = (csr[number_mip][31 - 7] && csr[number_mie][31 - 7] && csr[number_mideleg][31 - 7] == 1 && number_this_priviledge < 2 && (number_this_priviledge < 1 || csr[number_mstatus][31 - 1] == 1)) ||
		(csr[number_mip][31 - 5] && csr[number_mie][31 - 5] && number_this_priviledge < 2 && (number_this_priviledge < 1 || csr[number_mstatus][31 - 1] == 1));
	// bool S_external_interrupt = (csr[number_mip][31-11] && csr[number_mie][31-11] && csr[number_mideleg][31-11]==1 && number_this_priviledge!=3 && (number_this_priviledge<1 || csr[number_mstatus][31-1]==1)) || (csr[number_mip][31-9] && csr[number_mie][31-9] && number_this_priviledge!=3 && (number_this_priviledge<1 || csr[number_mstatus][31-1]==1));
	bool S_external_interrupt = (csr[number_mip][31 - 11] && csr[number_mie][31 - 11] && csr[number_mideleg][31 - 11] == 1 && number_this_priviledge < 2 && (number_this_priviledge < 1 || csr[number_mstatus][31 - 1] == 1)) ||
		(csr[number_mip][31 - 9] && csr[number_mie][31 - 9] && number_this_priviledge < 2 && (number_this_priviledge < 1 || csr[number_mstatus][31 - 1] == 1)); // || (csr[number_sstatus][31-1]==true && csr[number_sie][31-9] == true && csr[number_sip][31-9] == true);
	bool ecall = (number_op_code == 0x73 && number_funct3_unsigned == 0 && cvt_bit_to_number_unsigned(bit_csr_code, 12) == 0);
	bool illegal_exception = (number_csr_code_unsigned == number_timeh || number_csr_code_unsigned == number_time);
	...
	bool STrap = S_software_interrupt
			...
			||	(ecall 	&& (number_this_priviledge==0) && csr[number_medeleg][31- 8])
			||	(ecall 	&& (number_this_priviledge==1) && csr[number_medeleg][31- 9])
			//||	(ecall 	&& (number_this_priviledge==3) && csr[number_medeleg][31-11]) //M态ECALL无论如何不会进入STrap
			|| 	(page_fault_inst && csr[number_medeleg][31-12])		
			|| 	(page_fault_load && csr[number_medeleg][31-13])		
			|| 	(page_fault_store && csr[number_medeleg][31-15]);
	bool info[32] = {0};
	if (page_fault_inst || page_fault_load || page_fault_store) {
		if (page_fault_inst) copy_indice(info, 0, bit_this_pc, 0, 32);
		else if (number_op_code == number_11_opcode_lrw) copy_indice(info, 0, bit_reg_data_a, 0, 32);
	...
	if (MTrap){
		...
		// cout << (ecall 	&& (number_this_priviledge==3)) << endl;
		...
		if (csr[number_mtvec][31]==1 && csr[number_mtvec][30]==0 && bit_interruptType[0]==1) {
			init_indice(bit_next_pc, 0, 32);
		...
		if (page_fault_inst || page_fault_load || page_fault_store){//next_mtval = ;//要看具体实现
		...
		copy_indice(next_csr[number_sstatus], 31-12, this_priviledge, 0, 2);//next_mstatus.MPP = this_priviledge
		next_csr[number_sstatus][31-7] = csr[number_sstatus][31-3];//next_mstatus.MPIE = mstatus.MIE;
		...
		uint32_t number_interruptType = cvt_bit_to_number_unsigned(bit_interruptType,32);
		number_interruptType += (S_external_interrupt || (ecall && (number_this_priviledge==1) && csr[number_medeleg][31- 9])) ? 9
								: S_timer_interrupt ? 5
								: (ecall 	&& (number_this_priviledge==0) && csr[number_medeleg][31- 8]) ? 8
		...
		if (csr[number_stvec][31]==1 && csr[number_stvec][30]==0 && bit_interruptType[0]==1) {
			init_indice(bit_next_pc, 0, 32);
		...
		if (page_fault_inst || page_fault_load || page_fault_store){///next_stval = 0;//要看具体实现
		...
		next_csr[number_sstatus][31-3] = csr[number_sstatus][31-7]; //next_mstatus.MIE = mstatus.MPIE;
		...
		next_csr[number_sstatus][31-12] = 0; next_csr[number_sstatus][31-11] = 0;//next_mstatus.MPP = U;
		...
		switch (number_csr_code_unsigned) {
			case number_sie: {
				...
				uint32_t number_csr_data_result = cvt_bit_to_number_unsigned(bit_csr_data_result, 32);
				number_csr_data_result = (number_csr_data & 0xfffffccc) | (number_csr_data_result & 0x00000333);
				cvt_number_to_bit_unsigned(bit_csr_data_result, number_csr_data_result, 32);
			...
			case number_sstatus: {
				uint32_t number_csr_data = cvt_bit_to_number_unsigned(bit_csr_data, 32);
				uint32_t number_csr_data_result = cvt_bit_to_number_unsigned(bit_csr_data_result, 32);
				number_csr_data_result = (number_csr_data & 0x7ff21ecc) | (number_csr_data_result & (~0x7ff21ecc));
				cvt_number_to_bit_unsigned(bit_csr_data_result, number_csr_data_result, 32);
			...
			case number_mie: {
				uint32_t number_csr_data = cvt_bit_to_number_unsigned(bit_csr_data, 32);
				uint32_t number_csr_data_result = cvt_bit_to_number_unsigned(bit_csr_data_result, 32);
				number_csr_data_result = (number_csr_data & 0xfffff444) | (number_csr_data_result & 0x00000bbb);
				cvt_number_to_bit_unsigned(bit_csr_data_result, number_csr_data_result, 32);
				copy_indice(next_csr[number_sie], 0, bit_csr_data_result, 0, 32);
				copy_indice(next_csr[number_mie], 0, bit_csr_data_result, 0, 32);
				break;
			...
			case number_mstatus: {
				uint32_t number_csr_data = cvt_bit_to_number_unsigned(bit_csr_data, 32);
				uint32_t number_csr_data_result = cvt_bit_to_number_unsigned(bit_csr_data_result, 32);
				number_csr_data_result = (number_csr_data & 0x7f800644) | (number_csr_data_result & (~0x7f800644));
				cvt_number_to_bit_unsigned(bit_csr_data_result, number_csr_data_result, 32);
				copy_indice(next_csr[number_sstatus], 0, bit_csr_data_result, 0, 32);
				copy_indice(next_csr[number_mstatus], 0, bit_csr_data_result, 0, 32);
				break;
