...
void RISCV_CSR(bool input_data[bit_width], bool *output_data) {
  ...
  if (MTrap) {
    ...
    // 						: S_timer_interrupt ? 7
    // 						: (ecall 	&&
    // (number_this_priviledge==0) && csr[number_medeleg][31- 8]) ? 8 						: (ecall
    // && (number_this_priviledge==1) && csr[number_medeleg][31- 9]) ? 9 						:
    // (S_external_interrupt ||(ecall 	&& (number_this_priviledge==3) &&
    // csr[number_medeleg][31-11]))  ? 11 						: (page_fault_inst &&
    // csr[number_medeleg][31-12]) ? 12 						: (page_fault_load &&
    // csr[number_medeleg][31-13]) ? 13 						: (page_fault_store &&
    // csr[number_medeleg][31-15]) ? 15 						: 0 ;//给后31位赋值
    cvt_number_to_bit_unsigned(bit_interruptType, number_interruptType, 32);
    ...
    case 0: {                   // sfence.vma
      ...
      // csr[number_mepc], 0, 32); 	init_indice(next_csr[number_mcause], 0, 32);
      // }
      break;
    }
    case 1: { // csrrw
