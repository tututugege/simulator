...
{
    out.dcache2ldq_resp->valid = hit_ld && s2_reg_ld.valid && !global_mispred2 && !global_flush;
...
void Dcache::comb_s2()
{
    ...
    if (s2_reg_st.valid)
    {
        hit_st = false;
        hit_data_st = 0;
        hit_check(s2_reg_st.index, s2_reg_st.tag, hit_st, hit_way_st, hit_data_st, tag_next_st, data_next_st);
        new_data = write_data_mask(data_next_st[hit_way_st], s2_reg_st.wdata, s2_reg_st.wstrb);
    }
    ...
    if (DEBUG)
    {
        ...
        {
            printf("Dcache Store Access Debug Addr in S1 Stage: valid=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, tag=0x%08x, index=0x%02X, wdata=0x%08x, wstrb=0x%X  sim_time:%lld\n",
                   in.stq2dcache_req->en, in.stq2dcache_req->uop.instruction, in.stq2dcache_req->uop.rob_idx, in.stq2dcache_req->addr, GET_TAG(in.stq2dcache_req->addr), GET_INDEX(in.stq2dcache_req->addr), in.stq2dcache_req->wdata, in.stq2dcache_req->wstrb, sim_time);
        }
        if (s1_reg_st.valid && ((s1_reg_st.addr & 0xfffffffc) == DEBUG_ADDR))
        {
            printf("Dcache Store Access Debug Addr in S2 Stage: valid=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, tag=0x%08x, index=0x%02X, wdata=0x%08x, wstrb=0x%08X  sim_time:%lld\n",
                   s1_reg_st.valid, s1_reg_st.uop.instruction, s1_reg_st.uop.rob_idx, s1_reg_st.addr, s1_reg_st.tag, s1_reg_st.index, s1_reg_st.wdata, s1_reg_st.wstrb, sim_time);
        }
    }
}
void Dcache::comb_s1()
{
    ...
    else
    {
        ...
        if (DCACHE_LOG)
        {
            printf("Dcache Store New Request in S1 Stage: valid=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, tag=0x%08x, index=0x%02X, wdata=0x%08x, wstrb=0x%X \n",
                   s1_next_st.valid, s1_next_st.uop.instruction, s1_next_st.uop.rob_idx, s1_next_st.addr, s1_next_st.tag, s1_next_st.index, s1_next_st.wdata, s1_next_st.wstrb);
        }
    }
}
void Dcache::seq()
{
    ...
    if(DEBUG){
        for(int i=0;i<DCACHE_WAY_NUM;i++){
            if(dcache_tag[GET_INDEX(DEBUG_ADDR)][i]==GET_TAG(DEBUG_ADDR)&&dcache_valid[GET_INDEX(DEBUG_ADDR)][i]&&old_debug_data!=dcache_data[GET_INDEX(DEBUG_ADDR)][i][GET_OFFSET(DEBUG_ADDR)]){
                old_debug_data = dcache_data[GET_INDEX(DEBUG_ADDR)][i][GET_OFFSET(DEBUG_ADDR)];
                printf("\nDebug Dcache Read at addr:0x%08x data:0x%08x way:%d sim_time:%lld\n",DEBUG_ADDR,old_debug_data, i, sim_time);
                debug_way=i;
            }
        }
        if(debug_way!=-1&&dcache_tag[GET_INDEX(DEBUG_ADDR)][debug_way]!=GET_TAG(DEBUG_ADDR)){
            printf("\nDebug Dcache Read Miss at addr:0x%08x debug_way:%d dcache_tag:%d get_tag:%d sim_time:%lld\n",DEBUG_ADDR, debug_way, dcache_tag[GET_INDEX(DEBUG_ADDR)][debug_way], GET_TAG(DEBUG_ADDR), sim_time);
            debug_way=-1;
        }
    }
    ...
    if (hit_st && s2_reg_st.valid)
    {
        ...
        if (s2_reg_st.tag == s1_reg_st.tag && s2_reg_st.index == s1_reg_st.index&&GET_OFFSET(s2_reg_st.addr)==GET_OFFSET(s1_reg_st.addr))
        {
            ...
            if(DCACHE_LOG){
                printf("Dcache Store Forwarding in S1 Stage ST: way=%d new_data=0x%08X\n", hit_way_st, new_data);
            }
        }
    }
...
void Dcache::print()
{
    ...
    for (int i = 0; i < DCACHE_WAY_NUM; i++)
    {
        printf("Dcache Line[%02X] Way[%d]: Valid=%d Dirty=%d LRU=%d Tag=0x%08X Data[0]=0x%08X Data[1]=0x%08X Data[2]=0x%08X Data[3]=0x%08X \n",
               debug_index, i, dcache_valid[debug_index][i], dcache_dirty[debug_index][i], dcache_lru[debug_index][i], dcache_tag[debug_index][i],
               dcache_data[debug_index][i][0], dcache_data[debug_index][i][1],
               dcache_data[debug_index][i][2], dcache_data[debug_index][i][3]);
    }
    ...
    printf("Dcache Load S2: valid=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, tag=0x%08x, index=0x%02X, wdata=0x%08x, wstrb=0x%X \n",
           s2_reg_ld.valid, s2_reg_ld.uop.instruction, s2_reg_ld.uop.rob_idx, s2_reg_ld.addr, s2_reg_ld.tag, s2_reg_ld.index, s2_reg_ld.wdata, s2_reg_ld.wstrb);
    printf("Dcache Store S1: valid=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, tag=0x%08x, index=0x%02X, wdata=0x%08x, wstrb=0x%X \n",
           s1_reg_st.valid, s1_reg_st.uop.instruction, s1_reg_st.uop.rob_idx, s1_reg_st.addr, s1_reg_st.tag, s1_reg_st.index, s1_reg_st.wdata, s1_reg_st.wstrb);
    printf("Dcache Store S2: valid=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, tag=0x%08x, index=0x%02X, wdata=0x%08x, wstrb=0x%X \n",
           s2_reg_st.valid, s2_reg_st.uop.instruction, s2_reg_st.uop.rob_idx, s2_reg_st.addr, s2_reg_st.tag, s2_reg_st.index, s2_reg_st.wdata, s2_reg_st.wstrb);

    printf("Dcache Input Requests:\n");
    printf("  Load Queue Request: en=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, wdata=0x%08x, wstrb=0x%X \n",
           in.ldq2dcache_req->en, in.ldq2dcache_req->uop.instruction, in.ldq2dcache_req->uop.rob_idx, in.ldq2dcache_req->addr, in.ldq2dcache_req->wdata, in.ldq2dcache_req->wstrb);
    printf("  Store Queue Request: en=%d, uop_inst=0x%08x, rob_idx=%d addr=0x%08x, wdata=0x%08x, wstrb=0x%X \n",
           in.stq2dcache_req->en, in.stq2dcache_req->uop.instruction, in.stq2dcache_req->uop.rob_idx, in.stq2dcache_req->addr, in.stq2dcache_req->wdata, in.stq2dcache_req->wstrb);

    printf("Dcache Output Responses:\n");
    printf("  Load Queue Response: valid=%d, uop_inst=0x%08x, rob_idx=%d wen=%d, addr=0x%08x, data=0x%08x\n",
           out.dcache2ldq_resp->valid, out.dcache2ldq_resp->uop.instruction, out.dcache2ldq_resp->uop.rob_idx, out.dcache2ldq_resp->wen, out.dcache2ldq_resp->addr, out.dcache2ldq_resp->data);
    printf("  Store Queue Response: valid=%d, uop_inst=0x%08x, rob_idx=%d wen=%d, addr=0x%08x, data=0x%08x\n",
           out.dcache2stq_resp->valid, out.dcache2stq_resp->uop.instruction, out.dcache2stq_resp->uop.rob_idx, out.dcache2stq_resp->wen, out.dcache2stq_resp->addr, out.dcache2stq_resp->data);

    printf("DCache Output MSHR Requests:\n");
    printf("  Load MSHR Request: valid=%d, wen=%d, addr=0x%08x, uop_inst=0x%08x, rob_idx=%d\n",
           out.dcache2mshr_ld->valid, out.dcache2mshr_ld->wen, out.dcache2mshr_ld->addr, out.dcache2mshr_ld->uop.instruction, out.dcache2mshr_ld->uop.rob_idx);
    printf("  Store MSHR Request: valid=%d, wen=%d, addr=0x%08x, uop_inst=0x%08x, rob_idx=%d\n",
           out.dcache2mshr_st->valid, out.dcache2mshr_st->wen, out.dcache2mshr_st->addr, out.dcache2mshr_st->uop.instruction, out.dcache2mshr_st->uop.rob_idx);
