...
void MEMORY::seq()
{
    if (state == TRANSFER)
    {
        uint32_t addr_offset = ((in.control->addr >> 2) & (0xffffffff << in.control->size) | data_cnt) << 2;
        ...
        else
        {
            if(in.control->done){
                ...
                p_memory[addr_offset>>2] = (mask & in.control->wdata) | (~mask & old_data);
                data_cnt++;
                if(DEBUG){
                    if(addr_offset == DEBUG_ADDR){
                        printf("\nDebug Memory Write at addr:0x%08x wdata:0x%08x old_data:0x%08x mask:%08x p_memory:0x%08x sim_time:%lld\n",addr_offset,in.control->wdata,old_data,mask,p_memory[addr_offset>>2], sim_time);
                    }
                }
                if(DCACHE_LOG){
    ...
    if (state != TRANSFER || (in.control->wen==1&&in.control->done&&in.control->last))
    {
        data_cnt = 0;
    }
    ...
    if(DCACHE_LOG){
        ...
        printf("Memory Control: en=%d, wen=%d, sel=0x%02X, len=%u, done=%d, last=%d, size=%d, addr=0x%08x, wdata=0x%08x\n",
               in.control->en, in.control->wen, in.control->sel, in.control->len, in.control->done, in.control->last, in.control->size, in.control->addr, in.control->wdata);
    ...
    else if (in.control->en == true && Latency_cnt == Latency && state == LATENCY)
    {
        state = TRANSFER;
    }
    else if (in.control->en == true && in.control->wen==0 && data_cnt == in.control->len + 1 && state == TRANSFER)
    { // AXI优化
        state = MEM_IDLE;
    }
    else if(in.control->en == true && in.control->wen==1 && in.control->done && in.control->last && state == TRANSFER)
    {
        state = MEM_IDLE;
    }
