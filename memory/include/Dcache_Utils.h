...
#define GET_INDEX(addr) (((addr) >> (DCACHE_OFFSET_BITS + 2)) & ((1 << DCACHE_INDEX_BITS) - 1))
#define GET_OFFSET(addr) (((addr) >> 2) & ((1 << DCACHE_OFFSET_BITS) - 1))
#define GET_ADDR(tag, index, offset) ((tag << (DCACHE_INDEX_BITS + DCACHE_OFFSET_BITS) | index << DCACHE_OFFSET_BITS | offset) << 2)
