...
bool va2pa(bool* p_addr, bool* satp, bool* v_addr, uint32_t* p_memory, uint32_t type, bool* mstatus, uint32_t privilege, bool* sstatus) {
    uint32_t d = 24;
    uint32_t a = 25;
    uint32_t g = 26;
    uint32_t u = 27;
    ...
    if (log) cout << "pte1:" << hex << number_pte1_stored << endl;
    ...
    if (bit_pte1_stored[r]==true || bit_pte1_stored[x]==true) {
        if ((type == 0 && bit_pte1_stored[x]==true) 
            ...
            return false;
        if (privilege==1 && sum==0 && bit_pte1_stored[u]==true && sstatus[31-18]==false) return false;
        if (privilege!=1 && mprv==1 && mpp==1 && sum==0 && bit_pte1_stored[u]==true && sstatus[31-18]==false) return false;
        if ((number_pte1_stored >> 10) % 1024 != 0) return false;
        if (bit_pte1_stored[a]==false || (type==2 &&bit_pte1_stored[d]==false)) return false;
    ...
    if (log) cout << "pte2: " << hex << number_pte2_stored << endl;
    ...
    if (bit_pte2_stored[r]==true || bit_pte2_stored[x]==true) {
        if ((type == 0 && bit_pte2_stored[x]==true) 
            ...
            return false;
        if (privilege==1 && sum==0 && bit_pte2_stored[u]==true && sstatus[31-18]==false) return false;
        if (privilege!=1 && mprv==1 && mpp==1 && sum==0 && bit_pte2_stored[u]==true && sstatus[31-18]==false) return false;
        if (bit_pte2_stored[a]==false || (type==2 &&bit_pte2_stored[d]==false)) return false;
