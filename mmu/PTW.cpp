...
PTW::PTW(
    ...
    mmu_state_t *mmu_state_ptr) : in{
                                      .tlb2ptw = tlb2ptw_ptr,
                                      .dcache_resp = resp_m,
                                      .dcache_req = req_s},
                                  out{.ptw2tlb = ptw2tlb_ptr, .dcache_req = req_m, .dcache_resp = resp_s}, mmu_state{mmu_state_ptr}, pte1{}, pte2{}
{
  ...
  *out.ptw2tlb = {};
  ...
  case CACHE_1:
    ...
    if (dcache_state == DCACHE_IDLE)
    {
      ...
      uint32_t vindex = (vpn1 & 0x3FF) << 2; // 12 bits 虚拟索引 (实际上是实地址低位索引)
      ...
      if (read_resp_hs)
      {
        ...
        else
        {
          ...
          if (is_page_fault(pte1, op_type, true, &is_megapage))
          {
            ptw_state_next = IDLE;
            // refill tlb
            out.ptw2tlb->entry.set_valid_pte(pte1, mmu_state->satp.asid, vpn1, vpn0, is_megapage);
  ...
  case MEM_1:
    ...
    {
      // 访问内存，加载一级页表项
      uint32_t pte1_addr = ((mmu_state->satp.ppn & 0xFFFFF) << 10) | (vpn1 & 0x3FF);
      uint32_t pte1_number = p_memory[((mmu_state->satp.ppn & 0xFFFFF) << 10) | (vpn1 & 0x3FF)]; // 计算一级页表项的物理地址
  ...
  case CACHE_2:
    ...
    if (dcache_state == DCACHE_IDLE)
    {
      ...
      uint32_t vindex = (vpn0 & 0x3FF) << 2;         // 12 bits 虚拟索引 (实际上是实地址低位索引)
      out.dcache_req->paddr = (ptag << 12) | vindex;
      ...
      if (read_resp_hs)
      {
        ...
        else
        {
          ...
          if (is_page_fault(pte2, op_type, false, &is_megapage))
          {
            ptw_state_next = IDLE;
            // refill tlb
            out.ptw2tlb->entry.set_valid_pte(pte2, mmu_state->satp.asid, vpn1, vpn0, is_megapage);
            out.ptw2tlb->write_valid = true;
          }
          else if (is_megapage)
          {
            ...
            // out.ptw2tlb->entry.set_valid_pte(pte2, mmu_state->satp.asid, vpn1, vpn0, is_megapage);
            // out.ptw2tlb->write_valid = true;
            cout << "[PTW::comb] MEM_2 megapage refill phase: MEM_2 -> IDLE" << endl;
  ...
  case MEM_2:
    ...
    else
    {
      ...
      uint32_t pte2_number = p_memory[((pte2_ppn) & 0xFFFFF) << 10 | (vpn0 & 0x3FF)]; // 计算二级页表项的物理地址
      pte2 = *reinterpret_cast<pte_t *>(&pte2_number);
      // 根据是否 page fault 来决定下一个状态
      bool is_megapage = false;
      if (is_page_fault(pte2, op_type, false, &is_megapage))
  ...
  // {
  //   printf("\nPTW::comb state=%s next=%s dcache_state=%s next=%s sim_time=%lld\n",
  ...
  //          sim_time);
  //   printf("out.ptw2dcache_req->valid=%d out.ptw2dcache_req->paddr=0x%08X sim_time:%lld\n", out.dcache_req->valid, out.dcache_req->paddr, sim_time);
  //   printf("out.ptw2dcache_resp->ready=%d sim_time:%lld\n", out.dcache_resp->ready, sim_time);
  //   printf("in.dcache2ptw_resp->valid=%d in.dcache2ptw_resp->miss=%d in.dcache2ptw_resp->data=0x%08X sim_time:%lld\n", in.dcache_resp->valid, in.dcache_resp->miss, in.dcache_resp->data, sim_time);
  //   printf("in.dcache2ptw_resp->valid=%d sim_time:%lld\n", in.dcache_resp->valid, sim_time);
  // }
}
...
bool PTW::is_page_fault(pte_t pte, uint8_t op_type, bool stage_1, bool *is_megapage)
{
  ...
  if (pte.read == true || pte.execute == true)
  {
    ...
    if (privilege == 1 && sum_m == false && pte.user == true && sum_s == false)
    {
      return true;
    }

    // 非 S 模式下，MPRV 启用且 MPP 为 S 模式时不允许访问 U 模式可访问的页面
    if (privilege != 1 && mprv == true && mpp == 1 && sum_m == false && pte.user == true && sum_s == false)
    {
      return true;
    }
