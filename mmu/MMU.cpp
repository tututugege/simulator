...
extern long long sim_time; // simulation time for debug
...
void MMU::comb_frontend() {
   ...
   */
  if (io.in.state.satp.mode == MODE_BARE || 
  ...
  if (tlb.ifu_io.out.hit) {
    ...
    // check page fault
    bool page_fault = entry.is_page_fault(
      OP_FETCH,
      io.in.state.mstatus,
      io.in.state.sstatus,
