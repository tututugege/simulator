...
const string csr_names[21] = {
	"mtvec","mepc","mcause","mie","mip",
	"mtval","mscratch","mstatus","mideleg","medeleg",
	"sepc","stvec","scause","sscratch","stval",
	"sstatus","sie","sip","satp","mhartid",
	"misa"};

void print_regs(bool* arr) {
	for (int i = 0; i < 32; i++) {
		if (i % 10 == 0)
			cout << endl;
		bool reg[32];
...
void write_bool_to_file(ofstream & file, bool* arr, uint32_t num) {
	char temp[4] = {0};
