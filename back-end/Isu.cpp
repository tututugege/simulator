 ...
 * 输入依赖: in.dis2iss->req[][], configs[i].dispatch_width,
 * out.iss_awake（用于入队前叠加唤醒）, 当前 IQ 状态。 输出更新:
 * iqs[i].entry_1/count_1（通过 enqueue 写入）。 约束: 仅对 valid
 * 请求入队；入队失败视为设计错误并触发 Assert。
 */
void Isu::comb_enq() {
  for (int i = 0; i < IQ_NUM; i++) {
    auto &q = iqs[i];
    ...
    for (int w = 0; w < max_w; w++) {
      ...
      if (in.dis2iss->req[i][w].valid) {
        IqStoredUop uop =
            IqStoredUop::from_dis_iss_uop(in.dis2iss->req[i][w].uop);
        // 本拍入队前bypass唤醒总线
 ...
 * 输入依赖: q.schedule() 结果, in.exe2iss->ready, in.exe2iss->fu_ready_mask,
 * in.rob_bcast->flush, in.dec_bcast->mispred。 输出更新:
 * out.iss2prf->iss_entry[], 各 IQ 的已发射条目提交结果（commit_issue）。 约束:
 * 发射需同时满足端口 ready 与 FU 能力掩码；flush/mispred 时禁止新发射。
 */
void Isu::comb_issue() {
  ...
  for (size_t i = 0; i < iqs.size(); i++) {
    ...
    for (auto &pair : scheduled_pairs) {
      ...
      // 检查下游反压
      uint64_t req_bit =
          (1ULL << static_cast<uint32_t>(q.entry[entry_idx].uop.op));
      if (in.exe2iss->ready[phys_port] &&
        ...
        out.iss2prf->iss_entry[phys_port].uop =
            q.entry[entry_idx].uop.to_iss_prf_uop();
 ...
 * 输出更新: latency_pipe_1。
 * 约束: 仅 latency>1 且 dest_en 指令进入延迟管线；countdown==0
 * 的旧条目不再保留到下一拍。
 */
 ...
 * 输入依赖: in.prf_awake, latency_pipe, out.iss2prf->iss_entry,
 * get_latency(op), iqs[]。 输出更新: iqs[].wakeup(...) 结果,
 * out.iss_awake->wake[]。 约束: 唤醒端口数不超过 MAX_WAKEUP_PORTS；仅单周期且非
 * LOAD/STA 的新发射进入快速唤醒。
 */
void Isu::comb_awake() {
 ...
 * latency_pipe_1, iqs[]。 输出更新: iqs[]（flush_all/flush_br/clear_br
 * 后状态）, latency_pipe/latency_pipe_1。 约束: flush 优先级高于
 * mispred；clear_mask 在 flush/mispred 处理后作用于存活条目。
 */
void Isu::comb_flush() {
