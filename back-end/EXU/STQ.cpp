...
void STQ::comb_out() {
  ...
  if (entry[deq_ptr].valid && entry[deq_ptr].commit && deq_num < STQ_NUM) {
    extern uint32_t *p_memory;
    uint32_t wdata = entry[deq_ptr].data;
    uint32_t waddr = entry[deq_ptr].addr;
    uint32_t wstrb;
    ...
    if (waddr == UART_BASE) {
      ...
      p_memory[0x10000000 / 4] =
          p_memory[0x10000000 / 4] & 0xfff0ffff | 0x00030000;
    } else if (waddr == 0xc201004 &&
               (entry[deq_ptr].data & 0x000000ff) == 0xa) {
      p_memory[0xc201004 / 4] = 0x0;
    } else {
      out.stq2cache_req->en = true;
      out.stq2cache_req->wen = true;
      out.stq2cache_req->wstrb = wstrb;
      out.stq2cache_req->wdata = wdata;
      out.stq2cache_req->addr = waddr;
      out.stq2cache_req->uop.instruction = entry[deq_ptr].inst;

      out.stq2cache_req->uop.tag = entry[deq_ptr].tag;
      out.stq2cache_req->uop.dest_preg = deq_ptr;
      out.stq2cache_req->uop.rob_idx = (cpu.back.rob.deq_ptr << 2);
      entry[deq_ptr].issued = 1;
      write_flag = 1;
    }
    if (write_flag == 0) {
      entry[deq_ptr].issued = 2;
      out.stq2cache_req->en = false;
    }
  } else {
    out.stq2cache_req->en = false;
    write_flag = 2;
...
void STQ::comb_in() {
  if (DCACHE_LOG) {
    printf("\nSTQ Comb In:\n");
    printf("in.cache2stq_ready->ready=%d out.stq2cache_req->en=%d dest_preg:%d "
           "write_flag=%d\n",
           in.cache2stq_ready->ready, out.stq2cache_req->en,
           out.stq2cache_req->uop.dest_preg, write_flag);
    printf("in.cache2stq->valid=%d uop_inst=0x%08x dest_preg=%d\n",
           in.cache2stq->valid, in.cache2stq->uop.instruction,
           in.cache2stq->uop.dest_preg);
    for (int i = 0; i < STQ_NUM; i++) {
      printf("  STQ[%d]: valid=%d commit=%d issued=%d inst=0x%08x addr=0x%08x "
             "data=0x%08x size=0b%02x data_valid=%d addr_valid=%d\n",
             i, entry[i].valid, entry[i].commit, entry[i].issued, entry[i].inst,
             entry[i].addr, entry[i].data, entry[i].size, entry[i].data_valid,
             entry[i].addr_valid);
    }
    for (int i = 0; i < ROB_NUM; i++) {
      ...
      if (cpu.back.rob.entry[bank_idx][line_idx].valid &&
        ...
        printf("ROB entry %d maps to STQ entry %d data:%08x addr:%08x\n", i,
               stq_idx, entry[stq_idx].data, entry[stq_idx].addr);
      }
    }
    printf("enq_ptr=%d deq_ptr=%d fwd_ptr=%d count=%d commit_count=%d "
           "deq_num=%d\n",
  ...
  if (entry[fwd_ptr].issued == 2 && entry[fwd_ptr].valid == true &&
      entry[fwd_ptr].commit == true) {
    entry[fwd_ptr].valid = false;
    entry[fwd_ptr].commit = false;
  ...
  if (in.cache2stq->valid) {
    entry[in.cache2stq->uop.dest_preg].issued = 2;
  }
}
void STQ::seq() {
