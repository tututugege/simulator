...
void FU::exec(Inst_uop &inst, Mem_REQ *&in, bool mispred) {
  ...
  if (ldu_work == false || is_sta_uop(inst.op)) {
    ...
    if (is_load_uop(inst.op) || is_sta_uop(inst.op)) {
      ...
      if (is_load_uop(inst.op) && cpu.back.stq.st2ld_stall(inst.rob_idx)) {
        return;
      }
    }
  }
  ...
  if (DCACHE_LOG) {
    printf("cycle=%d latency=%d inst=0x%08x rob_idx=%d ldu_work=%d\n", cycle,
           latency, inst.instruction, inst.rob_idx, ldu_work);
  }
  if (cycle >= latency) {
    if (is_load_uop(inst.op)) {
      if (!ldu_work) {
        resp = cpu.mmu.io.out.mmu_lsu_resp[mmu_lsu_slot_r.idx];
        page_fault = resp.valid && resp.excp;
        mmu_ppn = resp.ptag;
      }
      // ldu(inst);
      if (DCACHE_LOG) {
        printf("  FU exec: before ldu, page_fault=%d mmu_ppn=0x%08x "
               "inst=0x%08x rob_idx=%d\n",
               page_fault, mmu_ppn, inst.instruction, inst.rob_idx);
      }
      ldu_work = true;
      ldu(inst, page_fault, mmu_ppn, in);
    } else if (is_sta_uop(inst.op)) {
      mmu_resp_master_t resp_st =
          cpu.mmu.io.out.mmu_lsu_resp[mmu_lsu_slot_r.idx];
      bool page_fault_st = resp_st.valid && resp_st.excp;
      uint32_t mmu_ppn_st = resp_st.ptag;
      if (DCACHE_LOG) {
        printf("  FU exec: before stu_addr, page_fault=%d mmu_ppn=0x%08x "
               "inst=0x%08x rob_idx=%d\n",
...
void EXU::comb_exec() {
  ...
  for (int i = 0; i < ISSUE_WAY; i++) {
    ...
    if (inst_r[i].valid) {
#ifdef CONFIG_CACHE
      fu[i].exec(out.exe2prf->entry[i].uop, out.exe2cache,
      ...
      //   ldu_work=%d\n", i, inst_r[i].uop.instruction, inst_r[i].uop.rob_idx,
      //   inst_r[i].valid, inst_r[i].uop.op, ldu_work);
      // }
      if (i == IQ_LD)
        continue;
#else
...
void EXU::comb_latency() {
  ...
  //   in.cache2exe_ready->ready); for(int i = 0; i < ISSUE_WAY; i++){
  //     printf("  FU[%d]: cycle=%d, latency=%d, complete=%d\n", i, fu[i].cycle,
  //     fu[i].latency, fu[i].complete);
  //   }
  //   printf("\n");
  //   for(int i = 0; i < ISSUE_WAY; i++)
  //   printf("  inst_r[%d]: valid=%d uop_inst=0x%08x\n", i, inst_r[i].valid,
  //   inst_r[i].uop.instruction); printf("\nmispred: %d flush: %d\n",
  //   in.dec_bcast->mispred, in.rob_bcast->flush);
  // }
  // if (DCACHE_LOG)
  // {
  ...
  if (inst_r[IQ_LD].valid && ldu_work) {
    ...
    if (in.dec_bcast->mispred &&
        ((1 << inst_r[IQ_LD].uop.tag) & in.dec_bcast->br_mask)) {
      fu[IQ_LD].complete = true;
      fu[IQ_LD].cycle = 0;
      out.exe2prf->entry[IQ_LD].valid = false;
      ldu_work = false;
