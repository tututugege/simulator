...
void Dcache::stage2() {
    ...
    for (int byte = 0; byte < 4; byte++) {
        ...
        fwd_info_r3_io.fwd_byte[byte] = stq_fwd_data->s.fwd_byte_in[byte];
        fwd_info_r3_io.fwd_strb[byte] = stq_fwd_data->s.fwd_strb_in[byte];
    }
...
void Dcache::stage3() {
    bool abort_w3 = req_info_r3.abort || (mshr_info->s.cache_replace_in || cache_req->s.valid_in && cache_req->s.op_in == op_t::OP_LD) && req_info_r3.valid && req_info_r3.op == op_t::OP_ST && hit_info_r3.hit;
    ...
    if (req_info_r3.valid && req_info_r3.op == op_t::OP_LD && hit_info_r3.hit && !abort_w3) {
        ...
        for (int byte = 0; byte < 4; byte++)
            if (stq_fwd_data->s.fwd_strb_in[byte])
                cache_res->m.data_out[byte] = stq_fwd_data->s.fwd_byte_in[byte];
            else
