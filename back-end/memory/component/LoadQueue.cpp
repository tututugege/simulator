...
void Load_Queue::alloc() {
    bool ldq_ful = (tail_r + 1) % LDQ_SZ == head_r;
    if (ren2lsu->valid && ren2lsu->op == op_t::LD && !ldq_ful) {
...
void Load_Queue::free() {
    if (!rob2ldq->valid)
        return;
    for (int i = 0; i < rob2ldq->retire_num; i++) {
        ldq_io[(head_r + i) % 5].valid = false;
    }
    head_r_io = (head_r + rob2ldq->retire_num) % 5;
}
...
void Load_Queue::seq() {
    if (ren2lsu->valid && lsu2ren->ready && ren2lsu->dis_fire) {
        ...
        ldq_io[tail_r].abort      = false;
        ldq_io[tail_r].done       = false;
        ldq_io[tail_r].miss       = false;
        ldq_io[tail_r].bcast      = false;
        ldq_io[tail_r].addr_trans = false;
        ldq_io[tail_r].paddrv     = false;
        ldq_io[tail_r].mem_sz     = ren2lsu->mem_sz;
        ldq_io[tail_r].dst_entry  = ren2lsu->dst_preg;
        ldq_io[tail_r].sign       = ren2lsu->sign;
        ldq_io[tail_r].id         = ren2lsu->id;
        tail_r_io = (tail_r + 1) % LDQ_SZ;
    }
