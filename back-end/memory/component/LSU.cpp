    ...
    if (stage2_info_r.valid) {
        if (stage2_info_r.src != src_t::RS && stage2_info_r.op == op_t::OP_LD) {
            ...
            if (mmu_resp->s.okay_in) {
                ...
                ldq_fill_req->m.paddr_out = true;
            }
            else if (mmu_resp->s.miss_in) {
                ldq_fill_req->m.tag_out = stage2_info_r_io.vtag;
                ldq_fill_req->m.addr_trans_out = false;
                ldq_fill_req->m.paddr_out = false;
            }
            else {
            }
        }
            ...
            if (mmu_resp->s.okay_in) {
                ...
                stq_fill_req->m.paddr_out = true;
            }
            else if (mmu_resp->s.miss_in) {
                stq_fill_req->m.tag_out = stage2_info_r_io.vtag;
                stq_fill_req->m.addr_trans_out = false;
                stq_fill_req->m.paddr_out = false;
            }
            else {
            }
        }
