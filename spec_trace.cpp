...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < 1000000000; i++) { //10398623
		...
		// log
		// if (privilege == 0) {
		// 	bool bit_load_address_trace[32];
		// 	bool bit_store_address_trace[32];
		// 	copy_indice(bit_store_address_trace, 0, output_data_from_RISCV, POS_OUT_STORE_ADDR, 32);
		// 	copy_indice(bit_load_address_trace, 0, output_data_from_RISCV, POS_OUT_LOAD_ADDR, 32);
		// 	uint32_t number_load_address_trace = cvt_bit_to_number_unsigned(bit_load_address_trace, 32);
		// 	uint32_t number_store_address_trace = cvt_bit_to_number_unsigned(bit_store_address_trace, 32);
		// 	uint32_t number_inst = cvt_bit_to_number_unsigned(bit_inst, 32);
		// 	cout << hex << number_PC << "|" << number_inst << "|"<< number_load_address_trace << "|" << number_store_address_trace << endl;
		// }
		// log
	}
